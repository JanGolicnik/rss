...
// -----------------------------------------------------------------------------
// Visit counting: fire a background fetch to /go/<id> on click. The anchor's
// href points at the real URL, so bots and link previewers follow that and
// don't inflate visit counts. keepalive: true lets the request complete even
// as the new tab opens.
// -----------------------------------------------------------------------------
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (e.key === "j") {
      ...
      if (focusedIdx >= 0 && entries[focusedIdx]) {
        var link = entries[focusedIdx].querySelector("a[data-go]");
        if (link) {
          fetch(link.dataset.go, { method: "GET", keepalive: true }).catch(
            function () {},
          );
